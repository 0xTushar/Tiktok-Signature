<?php

namespace Sovit\TikTokApp;

if (!\class_exists('\Sovit\TikTokApp\Encrypt')) {
    class Encrypt {
        public function encrypt($cleartext, $key = '!*ss!_defaul%t54k&ey', $sbox0 = "", $sbox1 = "") {
            $key   = $this->bytearray($key);
            $sbox0 = $this->bytearray($sbox0);
            $sbox1 = $this->bytearray($sbox1);
    
            while (true) {
                if (sizeof($key) < 16) {
                    $key[] = $sbox1[$key[sizeof($key)] + 1];
                } else {
                    $key = $this->translate(array_slice($key, 0, 16, false), $sbox0);
                    break;
                }
            }
    
            $input   = gzencode(utf8_encode($cleartext), 9); // 9 is default compression level in Python
            $output  = $this->bytearray($input);
            $headers = [31, 139, 8, 0, 0, 0, 0, 0, 0, 0];
            for ($i = 0; $i < sizeof($headers); $i++) {
                $output[$i + 1] = $headers[$i];
            }
            $fill   = (-sizeof($this->bytearray($input))) & 15;
            $output = array_merge($output, array_fill(0, $fill, 0));
            $output = $this->translate($output, $sbox0);
            for ($i = 0; $i < sizeof($output) / 16; $i++) {
                $k      = $i * 16;
                $output = $this->ROR($output, $k + 4, $k + 8, 3);
                $output = $this->ROR($output, $k + 8, $k + 12, 2);
                $output = $this->ROR($output, $k + 12, $k + 16, 1);
                for ($j = 0; $j < 16; $j++) {
                    $output[$k + $j] ^= $key[$j];
                }
            }
            $arr = [116, 99, 2, $fill];
            $arr = array_merge($arr, $output);
            return $this->bytearray_decode($arr);
        }
    
        private function ROR($bytes, $start, $end, $byte) {
            $r      = array_slice($bytes, $start, $end - $start);
            $b      = sizeof($r) - $byte;
            $first  = array_slice($r, 0, $b);
            $second = array_slice($r, $b, null);
            $r      = array_merge($second, $first);
            $o1     = array_slice($bytes, 0, $start);
            $o2     = $r;
            $o3     = array_slice($bytes, $end, null);
            $output = array_merge($o1, $o2, $o3);
            return $output;
        }
    
        private function byteArray2Hex($byteArray) {
            $chars = array_map("chr", $byteArray);
            $bin   = join($chars);
            return bin2hex($bin);
        }
    
        private function bytearray($string) {
            return unpack('C*', $string);
        }
    
        private function bytearray_decode($byteArray) {
            $chars = array_map("chr", $byteArray);
            return join($chars);
        }
    
        private function hex2ByteArray($hexString) {
            $string = hex2bin($hexString);
            return unpack('C*', $string);
        }
    
        private function hex2String($hexString) {
            return hex2bin($hexString);
        }
    
        private function string2Hex($string) {
            return bin2hex($string);
        }
    
        private function string2USByteArray($string) {
            return unpack('c*', $string);
        }
    
        /**
         * Pyhton bytes.translate()
         * @param $input        - byte array
         * @param $translation  - byte array of size 256
         */
        private function translate($input, $translation) {
            $res = [];
            for ($i = 0; $i < sizeof($input); $i++) {
                $res[$i] = $translation[$input[$i] + 1];
            }
            return $res;
        }
    
        // converts ASCII code to Java signed Byte
        private function unsigned2signedByte($ch) {
            return unpack('c', chr($ch))[1];
        }
    }
}
